...
class Compiler {
  ...
  compile(callback) {
    // 虽然 webpack 只有一个 Compiler，但是每次编译都会产出一个新的 Compilation，
    // 这里主要是为了考虑到 watch 模式，它会在启动时先编译一次，然后监听文件变化，如果发生变化会重新开始编译
    // 每次编译都会产出一个新的 Compilation，代表每次的编译结果
  ...
  run(callback) {
    ...
    const onCompiled = (err, stats, fileDependencies) => {
      ...
      // 当编译成功后会触发 done 这个钩子执行
      this.hooks.done.call(); 
    }

    // 开始编译，成功之后调用 onCompiled
...
class Compilation {
  ...
  // 当编译模块的时候，name：这个模块是属于哪个代码块 chunk 的，modulePath：模块绝对路径
  buildModule(name, modulePath) {
    let sourceCodeStr = fs.readFileSync(modulePath, 'utf8');
    // buildModule 最终会返回一个 modules 模块对象，每个模块都会有一个 id, id 是相对于根目录的相对路径
    let moduleId = "./" + path.posix.relative(baseDir, modulePath); //模块 id: 从根目录出发，找到与该模块的相对路径（./src/index.js）
    // 6.2.2 创建模块对象
    let module = {
      id: moduleId,
    ...
    rules.forEach((rule) => {
      let { test } = rule;
      // 如果模块的路径和正则匹配，就把此规则对应的 loader 添加到 loader 数组中
    ...
    // 自右向左对模块进行转译
    const sourceCode = loaders.reduceRight((code, loader) => {
      return loader(code);
    }, sourceCodeStr);

    // 通过 loader 翻译后的内容一定得是 js 内容，因为最后得走我们 babel-parse，只有 js 才能成编译 AST
    ...
    traverse(ast, {
      CallExpression: (nodePath) => {
        ...
        if (node.callee.name === "require") {
          ...
          let extensions = this.options.resolve?.extensions || [ ".js" ]; //获取配置中的 extensions
  ...
  build(callback) {
    ...
    if (typeof this.options.entry === 'string') {
      entry.main = this.options.entry; // 如果是单入口，将 entry:"xx"变成{main:"xx"}，这里需要做兼容
    } else {
    ...
    for (const entryName in entry) {
      ...
      // 第八步：等所有模块都编译完成后，根据模块之间的依赖关系，组装代码块 `chunk`
    ...
    this.chunks.forEach((chunk) => {
      ...
      console.log('filename', this.assets)
...
function webpack(webpackOption) {
  // 第二步：用配置参数对象初始化 `Compiler` 对象
  const compiler  = new Compiler(webpackOption);

  // 第三步：挂载配置文件中的插件
  const { plugins } = webpackOption;
